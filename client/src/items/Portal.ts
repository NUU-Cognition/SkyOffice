...
import { openPortalDialog } from '../stores/PortalStore'
...
export default class Portal extends Item {
  ...
  onOverlapDialog() {
    if (this.currentUsers.size === 0) {
      this.setDialogBox('Press R to open portal')
    } else {
      this.setDialogBox('Press R to open portal')
    }
  }
  ...
  openDialog(network: Network) {
    if (!this.id) return
    store.dispatch(openPortalDialog(this.id))
    network.connectToPortal(this.id)
  }
}
